...
class gender_normalizer:
	...
	def gender_normalizer_load(self,file_name):
		#self.declesion_replacement = declesions
		#self.declesion_replacement.sort
		with open(file_name) as file:
			doc = xmltodict.parse( file.read())
			result = (doc["genderdeclensions"]["replacement"])
			'''
			...
			self.declesion_prefix = (doc["genderdeclensions"]["prefix"])	

	def print_normalizer(self):
	...
	def normalize_gender(self,token, tag):
		#choose biggest rule
		bigget_rule_size = 0
		...
		'''
		for index,elem in enumerate(self.declesion_targets):
			word = normalization[:-len(self.declesion_targets[index]):]
			...
			if( self.declesion_re[index].fullmatch(normalization)
				...
				and not (excep.fullmatch(normalization))):
					lemmatized_word = word + self.declesion_replacement[index]
