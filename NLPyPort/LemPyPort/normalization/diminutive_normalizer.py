...
class diminutive_normalizer:
	...
	def diminutive_normalizer_load(self,file_name):
		with open(file_name) as file:
			doc = xmltodict.parse( file.read())
			result = (doc["diminutivedeclensions"]["replacement"])
			'''
			...
			self.declesion_prefix = (doc["diminutivedeclensions"]["prefix"])	
					

	def print_diminutive_normalizer(self):
	...
	def normalize_diminutive(self,token, tag):
		bigget_rule_size = 0
		lemmatized_word = ""
		'''
